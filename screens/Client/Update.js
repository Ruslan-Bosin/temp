...
export default function ClientUpdate({navigation, route}) {

  let type = route.params.type;
  
  const [inputText, setInputText] = useState("");
  const [swithState, setSwitchState] = useState(route.params.is_private);
  ...
  const headerText = (type) => {
    ...
    if (type === "privacy") return "Настроки приватности"
  };

  const getBody = (type) => {
    if (type === "name") return JSON.stringify({name: inputText})
    if (type === "mail") return JSON.stringify({email: inputText})
    if (type === "password") return JSON.stringify({password: inputText})
    if (type === "privacy") return JSON.stringify({is_private: swithState})
  };
