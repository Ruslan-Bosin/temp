...
export default function OrganizationLogin({navigation}) {

  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [errorMessage, setErrorMessage] = useState("");

  const organizationSignnup = () => {
    navigation.navigate("OrganizationSignup");
  };

  const loginPressed = () => {
      
    fetch(Config.base_url + "/rest/organization/login", {
      method: "POST",
      headers: {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <Text style={styles.info}>
        Вход в аккаунт организации. Если у вас нет аккаунта вы можете
      </Text>
      <TouchableOpacity onPress={organizationSignnup}><Text style={styles.infoLink}>зарегистрироваться</Text></TouchableOpacity>
